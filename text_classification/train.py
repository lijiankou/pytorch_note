...
def train_and_valid(lr_, sub_train_, sub_valid_, model = '', loss_fun = ''):
    r"""
...
def test(data_, model = ''):
    ...
    for text, offsets, cls in data:
        ...
        with torch.no_grad():
            output = model(text, offsets)
            print(output.argmax(1) == cls).float().mean()
...
criterion
    ...
    train_and_valid(lr, sub_train_, sub_valid_, model = model, loss_fun = loss_fun)
